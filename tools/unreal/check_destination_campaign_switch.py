...
objects = [a for a in u.GameplayStatics.get_all_actors_of_class(world, u.CoastalWorldObject)
           if str(a.world_id).startswith(('world.north_reach.', 'world.coastal_records.', 'world.outer_coast.'))]
if len(objects) != 9 or not all(a.is_active() for a in objects):
    raise RuntimeError('All nine destination records must already be completed')
...
(Path(__file__).resolve().parents[3] / 'local-evidence/m3-outer-coast-campaign-switch.json').write_text(
    json.dumps({'passed': True, 'rows': rows, 'all_save_hashes_unchanged': True}, indent=2), encoding='utf-8')
print('Old/completed campaign switching resets and restores all nine world/journal pairs')
