"""Import the original zipline trolley and assign a steel material."""
import json
...
if u.EditorAssetLibrary.does_asset_exist(path):
    ...
    for key,value in dict(filename=str(ROOT/'local-evidence/m3-zipline-source/trolley.obj'),destination_path=folder,
        destination_name='SM_ZiplineTrolley',automated=True,replace_existing=False,save=True,options=options).items():
...
(ROOT/'local-evidence/m3-zipline-trolley-import.json').write_text(json.dumps(report,indent=2))
