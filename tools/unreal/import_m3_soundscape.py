"""Import only the nine selected private score/thunder WAVs into the independent UE5.8 host."""
import hashlib
import json
...
manifest = json.loads((ROOT / 'CoastalExploration/data/m3_soundscape.json').read_text())
staging = json.loads((ROOT / 'local-evidence/m3-soundscape-staging.json').read_text())
destination = manifest['destination']
effects = u.load_asset('/Game/Coastal/Audio/SC_M1Effects')
ambience = u.load_asset('/Game/Coastal/Audio/SC_M1Ambience')
if not effects or not ambience:
...
for clip in staging['clips']:
    ...
    sound.set_editor_property('sound_class_object', ambience if clip['loop'] else effects)
    sound.set_editor_property('virtualization_mode', u.VirtualizationMode.PLAY_WHEN_SILENT)
    sound.set_editor_property('looping', clip['loop'])
...
(ROOT / 'local-evidence/m3-soundscape-import.json').write_text(json.dumps(report, indent=2))
