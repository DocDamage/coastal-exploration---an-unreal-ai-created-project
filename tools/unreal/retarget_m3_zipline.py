"""Retarget the installed two-handed zipline loop; preserve prior action bindings."""
import json
...
sources = [('zipline_ride', 'Two_Handed_Zipline_Anim')]
actions = dict(definition.get_editor_property('actions'))
report = []
for cue, relative in sources:
    path = '/Game/FreeAnimationSet/Animations/Manny/' + relative
...
(ROOT / 'local-evidence/m3-zipline-retarget.json').write_text(json.dumps(report, indent=2))
